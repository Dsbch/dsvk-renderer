...
{
	engineCore::key engineCore::fromRawMouse(const RAWINPUT* raw)
	{
		if (!raw)
			return unknown;
	...
	engineCore::key engineCore::fromRawKeyboard(const RAWINPUT* raw)
	{
		if (!raw)
			return unknown;
	...
	engineCore::key engineCore::fromWinApiMouse(int msg)
	{
		switch (msg)
		{
	...
	engineCore::key engineCore::fromWinApiKey(int vkCode)
	{
		switch (vkCode) {
		case VK_ESCAPE:  return escape;
		case VK_RETURN:  return enter;
