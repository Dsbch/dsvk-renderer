...
namespace vktest
{
	...
	engine::error vulkanBuffer::build(immediateSubmit is, void* data, size_t sizeInBytes, size_t validBytes)
	{
		if (mBuffer.buffer != VK_NULL_HANDLE)
			return engine::error{ "buffer already created" };
	...
	size_t vulkanBuffer::geSize() const
	{
		return mByteSize;
	}
	...
	engine::error vulkanBuffer::updateBuffer(immediateSubmit is, void* data, size_t sizeInBytes, size_t offset)
	{
		if (sizeInBytes + mLoadedBytes >= mByteSize)
			return engine::error{"buffer overflow"};
