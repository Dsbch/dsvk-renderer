...
namespace engine
{
	...
	withError<lodMesh> loadMeshTest(const std::string& path, size_t maxVert, size_t maxTriangles, float coneWieght)
	{
		...
		auto err = loadMeshFromGLTF(path, v, i);
		if (err)
			return err;
		...
		const size_t maxMeshlets = meshopt_buildMeshletsBound(newIndex.size(), maxVert, maxTriangles);
		...
		meshlets.resize(maxMeshlets);
		meshletVertices.resize(maxMeshlets * maxVert);
		meshletTriangles.resize(maxMeshlets * maxTriangles * 3);
		...
		LOGINFO(
			"meshOptTest end vertexCount: {}, triagnleCount: {}, meshletsCount: {}, indexBufferCount: {}",
			newVert.size(),
			meshletTrianglesU32.size(),
			meshlets.size(),
		...
		result.vertexBuffer = std::make_shared<std::vector<vertex>>();
		result.lodLevels.front().indexBuffer = std::make_shared<std::vector<uint32_t>>();
		result.lodLevels.front().meshletBuffer = std::make_shared<std::vector<meshlet>>();
		result.lodLevels.front().primitiveBuffer = std::make_shared<std::vector<uint32_t>>();

		for (auto v : newVert)
			result.vertexBuffer->push_back(v);

		for (auto v : meshletVertices)
			result.lodLevels.front().indexBuffer->push_back(v);


		for (auto v : meshlets)
			result.lodLevels.front().meshletBuffer->push_back(
				meshlet{
						.indexBufferIndex = 0,
						.indexBufferOffset = v.vertex_offset,
						.vertexBufferIndex = 0,
						.vertexBufferOffset = 0,
						.vertexCount = v.vertex_count,
						.triangleBufferIndex = 0,
						.triangleBufferOffset = v.triangle_offset,
						.triangleCount = v.triangle_count
				}
			);


		for (auto v : meshletTrianglesU32)
			result.lodLevels.front().primitiveBuffer->push_back(v);
